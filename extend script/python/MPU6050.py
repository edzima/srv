...
class MPU6050Data:
  ...
  def round2(self):
    self.Gx = round(self.Gx,3)
    self.Gy = round(self.Gy,3)
    self.Gz = round(self.Gz,3)

    self.Gyrox = round(self.Gyrox,3)
    self.Gyroy = round(self.Gyroy,3)
    self.Gyroz = round(self.Gyroz,3)
  
  def delta(self,InitData):
      self.Gx = InitData.Gx - self.Gx
      self.Gy = InitData.Gy - self.Gy
      self.Gz = InitData.Gz - self.Gz

      self.Gyrox = InitData.Gyrox - self.Gyrox
      self.Gyroy = InitData.Gyroy - self.Gyroy
      self.Gyroz = InitData.Gyroz - self.Gyroz

  def force(self, InitData):
      CurrentForce = math.sqrt( (self.Gx - InitData.Gx) * ( self.Gx - InitData.Gx) +
                               (self.Gy - InitData.Gy) * ( self.Gy - InitData.Gy) +
                               (self.Gz - InitData.Gz) * ( self.Gz - InitData.Gz))
      return CurrentForce
...
class MPU6050:
   ...
   def detectVibration(self, name):
       ...
       for loop in range(20):
           ...
           # read the accelerometer and calculate new force
           Current = self.readData()
           CurrentForce = Current.force(I)
       ...
       if PeakForce > Threshold :
           if not(ShakeFlag) :
               ShakeFlag = True
               print name, ' Vibration detected ', PeakForce
               return PeakForce
       else:
           ShakeFlag= False
           return False
